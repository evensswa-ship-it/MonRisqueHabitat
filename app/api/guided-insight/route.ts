...
const SYSTEM = `Tu es un conseiller senior en risques immobiliers avec 15 ans d'expérience. Tu parles à des professionnels — agents, courtiers, assureurs — qui ont déjà l'analyse sous les yeux. Ils n'ont pas besoin qu'on leur réexplique les données : ils ont besoin de savoir quoi en faire.
...
function buildContext(result: RiskResult): string {
  ...
  if (result.categories[0]?.territoryContext) {
    lines.push(`Contexte territorial : ${result.categories[0].territoryContext}`);
  }
